...
const DEFAULT_OVERLAY_PATH = "data/integrations-sh-overlay.json";
const DEFAULT_OUT_PATH = "data/integrations-sh-registry.json";
...
Build a MCP registry JSON file from integrations.sh surface data.
...
async function fetchJson(url) {
  const response = await fetch(url, {
    headers: { accept: "application/json", "user-agent": "mcp-registry-sync" },
  });
  if (!response.ok) {
...
function mergeEntries(generated, overlay) {
  ...
  return [...byName.values()].sort((a, b) => {
    const aLabel = a.server.title || a.server.name;
    const bLabel = b.server.title || b.server.name;
